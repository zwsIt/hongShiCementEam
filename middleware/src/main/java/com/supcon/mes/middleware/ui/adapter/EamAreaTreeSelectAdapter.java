 ...
 * @Desc
 */
public class EamAreaTreeSelectAdapter extends /*BaseRecyclerViewAdapter*/BaseListDataRecyclerViewAdapter<ICustomTreeView<Area>> {
    private List<ICustomTreeView<Area>> contentList = new ArrayList<>();

    public void setRootEntity(ICustomTreeView<Area> customMultiStageEntity) {
        contentList.clear();
        initContentList(customMultiStageEntity);
        notifyDataSetChanged();
    }

    public void initContentList(ICustomTreeView<Area> rootEntity) {
    ...
    public EamAreaTreeSelectAdapter(Context context) {
        super(context);
    }
...
//        return getItem(position).getFullPathName().split("/").length;
    ...
    @Override
    public ICustomTreeView<Area> getItem(int position) {
    ...
    class TreeViewHolder extends BaseRecyclerViewHolder<ICustomTreeView<Area>> {
        ...
        protected void initListener() {
            ...
            RxView.clicks(itemView)
                    ...
                    .subscribe(o -> {
                        ...
                        ICustomTreeView<Area> customMultiStageEntity = getItem(getAdapterPosition());
                        customMultiStageEntity.changeExpandStatus();
                        Flowable<List<ICustomTreeView<Area>>> customMultiStageEntities = customMultiStageEntity.getChildNodeList();
                        customMultiStageEntities
                                .flatMap((Function<List<ICustomTreeView<Area>>,
                                        Publisher<ICustomTreeView<Area>>>)
                        ...
                        if (!customMultiStageEntity.isExpanded()) {
                            ...
                            List<ICustomTreeView<Area>> sub = new ArrayList<>();
        ...
        @Override
        protected void update(ICustomTreeView<Area> data) {
...
//                        "(" + data.getCurrentEntity().layRec + ")" : "")
        ...
        public void changeStatus(ICustomTreeView<Area> data) {
            if (data.isRootEntity()) {
//                areaIcon.setImageResource(R.drawable.ic_multi_home);
