...
/**
 * ClassName
 * Created by zhangwenshuai1 on 2020/1/13
 * Email zhangwenshuai1@supcon.com
 * Desc 送电列表
 */
@Router(value = Constant.Router.HS_SD_LIST)
...
public class ElectricityOnListActivity extends BaseRefreshRecyclerActivity<ElectricityOffOnEntity> implements ElectricityOffOnListContract.View {
    ...
    private ElectricityOffOnListAdapter mElectricityOffOnListAdapter;
    ...
    protected IListAdapter<ElectricityOffOnEntity> createAdapter() {
        mElectricityOffOnListAdapter = new ElectricityOffOnListAdapter(context);
        return mElectricityOffOnListAdapter;
    }

    @Override
    protected void initView() {
        super.initView();
        searchTitleBar.setTitleText("送电票");
        ...
        getController(WorkFlowKeyController.class).queryWorkFlowKeyAndPermission(Constant.EntityCode.ELE_ON_OFF, Constant.EntityCodeType.ELE_ON, new OnAPIResultListener<Object>() {
            @Override
...
//        getController(ModulePermissonCheckController.class).checkModulePermission(EamApplication.getUserName().toLowerCase(), ProcessKeyUtil.ELE_ON, result -> {
//            if (result == null){
//                searchTitleBar.disableRightBtn();
//            }
//            deploymentId = result;
//        },null);
    ...
    protected void initListener() {
        ...
        refreshListController.setOnRefreshPageListener(new OnRefreshPageListener() {
            ...
            public void onRefresh(int pageIndex) {
                presenterRouter.create(ElectricityOffOnListAPI.class).listElectricityOffOn(pageIndex,queryParam,pendingQuery,true);
            }
        });
        ...
        mElectricityOffOnListAdapter.setOnItemChildViewClickListener(new OnItemChildViewClickListener() {
            @Override
    ...
    private void goDetail(ElectricityOffOnEntity electricityOffOnEntity) {
        ...
        if (electricityOffOnEntity.getPending().id == null || electricityOffOnEntity.getPending().openUrl == null){ // 无代办、生效
            ...
            switch (electricityOffOnEntity.getPending().openUrl) {
                case Constant.HSEleOnView.EDIT_URL:
                    IntentRouter.go(context, Constant.Router.HS_ELE_ON_EDIT, bundle);
                    break;
                case Constant.HSEleOnView.PREVIEW_URL:
                default:
    ...
    private void dealGo(String code) {
        ...
        if (mElectricityOffOnListAdapter != null && mElectricityOffOnListAdapter.getList() != null && mElectricityOffOnListAdapter.getList().size() > 0){
            List<ElectricityOffOnEntity> tempEntityList = new ArrayList<>();
            List<String> tableNoList = new ArrayList<>();
            for (ElectricityOffOnEntity entity : mElectricityOffOnListAdapter.getList()){
