    ...
    private void queryEamFileViewUrl(Long id) {
        Api.getInstance().retrofit.create(ApiService.class).getEamFileViewUrl(id)
                ...
                .onErrorReturn(throwable -> {
                    ...
                    entity.errMsg = HttpErrorReturnUtil.getErrorInfo(throwable);
                    return entity;
                })
                ...
                .subscribe(eamFileViewUrlEntity -> {
                    if (eamFileViewUrlEntity.dealSuccess){
                        ...
                        onAPIResultListener.onFail(TextUtils.isEmpty(eamFileViewUrlEntity.errMsg)? "文件不存在或数据异常" : eamFileViewUrlEntity.errMsg);
                    }
                });
    }
